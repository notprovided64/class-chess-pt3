...
public:
  ...
  // Should return true if it is legal for the given bit to be moved from its
  // current holder. Default implementation always returns true.
  virtual bool canBitMoveFrom(Bit &bit, BitHolder &src) = 0;

  // /** Should return true if it is legal for the given Bit to move from src to
  // dst. Default implementation always returns true.
  ...
  // Should handle any side effects of a Bit's movement, such as captures or
  // scoring. Does not need to do the actual movement! That's already happened.
  ...
  // Should return NO if the click is illegal (i.e. clicking an empty draw pile
  // in a card game.) Default implementation always returns true.
  ...
  virtual bool animateAndPlaceBitFromTo(Bit &bit, BitHolder &src,
                                        BitHolder &dst);
  ...
  // this code below limits class to two players at the most, but it ensures
  // player 0 is white and player 1 is black
  Player *getCurrentPlayer() {
    return (_players.size() != 0) ? _players.at(_gameOptions.currentTurnNo & 1)
                                  : nullptr;
  };
  Player *getPlayerAt(unsigned int playerNumber) {
    return (_players.size() != 0) ? _players.at(playerNumber) : nullptr;
  };
