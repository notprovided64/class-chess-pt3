  ...
  for (int y = 0; y < _gameOptions.rowY; y++) {
    for (int x = 0; x < _gameOptions.rowX; x++) {
      ...
      if (holder.isMouseOver(pos)) {
        ...
        if (holder.canDropBitAtPoint(_dragBit, pos) &&
            canBitMoveFromTo(*_dragBit, *_oldHolder, holder)) {
          _dropTarget = &holder;
...
void Game::drawFrame() {
  ...
  //
  // paint the pieces second so they are always on top of the board as we move
  // them
  //
  for (int y = 0; y < _gameOptions.rowY; y++) {
    for (int x = 0; x < _gameOptions.rowX; x++) {
  ...
  for (int y = 0; y < _gameOptions.rowY; y++) {
    for (int x = 0; x < _gameOptions.rowX; x++) {
      BitHolder &holder = getHolderAt(x, y);
      if (holder.bit() && holder.bit()->getMoving() &&
          !holder.bit()->getPickedUp()) {
        holder.bit()->update();
        holder.bit()->paintSprite();
      }
    }
